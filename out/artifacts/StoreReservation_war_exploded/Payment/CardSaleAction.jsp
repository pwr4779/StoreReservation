...
<%
   ...
   if(CardCom == null || CardNo == null) {
%>
<script>
    alert("카드번호와 카드사 입력은 필수입니다.")
    location.href = 'PaymentProcessingView.jsp?OrderNo=<%=OrderNo%>'
</script>
<%
...
<%
    ...
    int result_1 = PayDAO.AffiliatePaymentInsert(OrderNo, 10, SaleCoupon, Amount, InstallMonth,UsingPoint); // 결제 테이블 (쿠폰 할인, 포인트사용)
    int result_2 = PayDAO.UserUpdateAboutPay(userDetail.getUserNo(),SaleCoupon,UsingPoint); // USER_DETAIL 갱신 -> 주문 수 +1, AC_POINT +1000, YEAR_AC_POINT +1000, 쿠폰수 차감, AC_Point 차감
    int result_3 = PayDAO.CardDiscountTableInsert(OrderNo, CardCom, CardNo); // 카드할인테이블 내역 삽입
    int result_4 = PayDAO.FinishPaymentProcessing(OrderNo); // 결제처리 완료 후 주문 테이블에 결제완료처리
